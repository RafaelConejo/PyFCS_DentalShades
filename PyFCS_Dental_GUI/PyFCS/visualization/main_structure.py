...
### my libraries ###
from PyFCS import Input, Visual_tools, ReferenceDomain, FuzzyColorSpace
...
class PyFCSApp:
    def __init__(self, root):
        ...
        root.title("PyFCS Dental Shades")  # Set the window title
        ...
        save_image = os.path.join(os.getcwd(), 'PyFCS_Dental_GUI', 'PyFCS', 'visualization', 'icons', 'SaveImage.png')
        save_image = Image.open(save_image)
        save_image = ImageTk.PhotoImage(save_image)

        new_fcs = os.path.join(os.getcwd(), 'PyFCS_Dental_GUI', 'PyFCS', 'visualization', 'icons', 'NewFCS1.png')
        new_fcs = Image.open(new_fcs)
        new_fcs = ImageTk.PhotoImage(new_fcs)

        load_fcs = os.path.join(os.getcwd(), 'PyFCS_Dental_GUI', 'PyFCS', 'visualization', 'icons', 'LoadFCS.png')
        load_fcs = Image.open(load_fcs)
        load_fcs = ImageTk.PhotoImage(load_fcs)

        at_image = os.path.join(os.getcwd(), 'PyFCS_Dental_GUI', 'PyFCS', 'visualization', 'icons', 'tooth.png')
        at_image = Image.open(at_image)
        at_image = ImageTk.PhotoImage(at_image)

        pt_image = os.path.join(os.getcwd(), 'PyFCS_Dental_GUI', 'PyFCS', 'visualization', 'icons', 'PT.png')
    ...
    def load_color_space(self):
        ...
        filename = utils_structure.prompt_file_selection(os.path.join('PyFCS_Dental_GUI', 'fuzzy_color_spaces'))
    ...
    def palette_based_creation(self):
        ...
        color_space_path = os.path.join(os.getcwd(), 'PyFCS_Dental_GUI', 'fuzzy_color_spaces', 'cns', 'ISCC_NBS_BASIC.cns')
    ...
    def on_add_graph(self, selected_options):
        ...
        file_path = os.path.abspath(os.path.join(os.getcwd(), 'PyFCS_Dental_GUI', 'PyFCS', 'external', 'temp_plot.html'))
    ...
    def open_image(self):
        ...
        initial_directory = os.path.join(initial_directory, 'PyFCS_Dental_GUI', 'image_test')
    ...
    def show_membership_results(self, window_id, img):
        ...
        try:
            ...
            tk.Label(result_window, text=f"Results for: {filename}", bg="white",
                    font=("Sans", 12, "bold")).pack(pady=(10, 10))
