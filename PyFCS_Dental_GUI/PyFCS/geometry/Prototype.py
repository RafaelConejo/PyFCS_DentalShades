...
class Prototype:
    ...
    def run_qvoronoi(self):
        ...
        try:
            ...
            command = f"PyFCS_Dental_GUI\\PyFCS\\external\\qvoronoi.exe Fi Fo p Fv"
            ...
            temp_output_file = os.path.join('PyFCS_Dental_GUI', 'PyFCS', 'external', 'temp', 'temp_voronoi_output.txt')
    ...
    def read_from_voronoi_file(self):
        ...
        """
        volumes = []
        file_path = os.path.join('PyFCS_Dental_GUI', "PyFCS", "external", "temp", "temp_voronoi_output.txt")
        points = np.vstack((self.positive, self.negatives))
